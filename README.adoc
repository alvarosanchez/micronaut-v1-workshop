...
ClubsApi : @Get("/{id}") List<Club> Club show(Long id)
....
endif::[]

image::clubs-diagram.png[]

icon:hand-o-right[] Create the `ClubsApi` interface, annotating its method with 
...
specified on `ClubsClient`. Use `ClubService` by declaring a constructor dependency
on it.
