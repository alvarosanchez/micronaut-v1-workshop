...
image::images/football-diagram.png[]

* `clubs` is the microservice responsible for managing clubs. It uses GORM for
  Hibernate as a data access layer.
* `fixtures` manages all game fixtures. For the teams playing in a game, it doesn't
  store their full details, but rather their ID. It has a service-discovery-enabled
  HTTP client to fetch club details from the `clubs` microservice.
